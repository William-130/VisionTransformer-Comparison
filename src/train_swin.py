    ...
    data_transforms = {
        "train": transforms.Compose(
            [
                ...
                # Random erasing to help regularization and reduce overfitting
...
def create_model(num_classes=5, pretrained=True):
    """
    Membuat Swin Transformer model dengan pre-trained weights.
    ...
    Returns:
        nn.Module: Swin Transformer model
    """
    print("\n" + "=" * 60)
    print("CREATING SWIN TRANSFORMER MODEL")
    print("=" * 60)

    # Load pre-trained Swin Tiny model dari timm
    model = timm.create_model(
        "swin_tiny_patch4_window7_224", pretrained=pretrained, num_classes=num_classes
    )
...
def save_model_and_history(model, history, model_name="swin"):
    """
...
def main():
    """
    Fungsi utama untuk menjalankan training Swin Transformer.
    """
    print("\n" + "=" * 60)
    print("SWIN TRANSFORMER TRAINING")
    print("Indonesian Food Classification")
    print("=" * 60)

    # Hyperparameters (adjusted for 4GB GPU)
    NUM_CLASSES = 5
    BATCH_SIZE = 8  # Reduced from 32 to fit in 4GB GPU
    NUM_EPOCHS = 10 
    LEARNING_RATE = 5e-6
    WEIGHT_DECAY = 0.1 
    ...
    save_model_and_history(model, history, model_name="swin")
