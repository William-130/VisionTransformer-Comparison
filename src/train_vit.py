...
def create_model(num_classes=5, pretrained=True):
    """
    Membuat Vision Transformer (ViT) model dengan pre-trained weights.
    ...
    # Load pre-trained ViT Base model dari timm
    # Model options:
    # - vit_base_patch16_224: ViT-B/16 (86M params)
    # - vit_small_patch16_224: ViT-S/16 (22M params) - lebih ringan
    # - vit_large_patch16_224: ViT-L/16 (304M params) - lebih besar
    ...
    print(f"  Model: Vision Transformer Base Patch16 224")
...
def train_model(
    ...
    Args:
        ...
        patience: Jumlah epoch untuk early stopping
...
def save_model_and_history(model, history, model_name="vit"):
    """
...
def main():
    """
    Fungsi utama untuk menjalankan training ViT.
    """
    print("\n" + "=" * 60)
    ...
    # Hyperparameters (adjusted for 4GB GPU)
    NUM_CLASSES = 5
    BATCH_SIZE = 4  # ViT Base is larger, needs even smaller batch size for 4GB GPU
    NUM_EPOCHS = 10 
    LEARNING_RATE = 5e-5  
    WEIGHT_DECAY = 0.1 
    ...
    save_model_and_history(model, history, model_name="vit")
